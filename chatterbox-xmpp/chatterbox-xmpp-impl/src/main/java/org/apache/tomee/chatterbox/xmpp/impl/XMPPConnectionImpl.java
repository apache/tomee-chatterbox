...
public class XMPPConnectionImpl implements XMPPConnection {
    private static Logger log = Logger.getLogger(XMPPConnectionImpl.class.getName());

    private XMPPManagedConnection mc;

    private XMPPManagedConnectionFactory mcf;

    public XMPPConnectionImpl(XMPPManagedConnection mc, XMPPManagedConnectionFactory mcf) {
        this.mc = mc;
        this.mcf = mcf;
    }

    public void sendMessage(String recipient, String message) throws MessageException {
        mc.sendMessage(recipient, message);
    }
